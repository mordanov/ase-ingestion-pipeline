...
async def test_register_response_has_required_fields(async_client: AsyncClient):
    ...
    required = {"device_id", "device_type", "model", "firmware_version", "os", "credit_balance", "reward_tier", "registered_at"}
...
async def test_register_response_no_pii(async_client: AsyncClient):
    ...
    assert not pii_fields.intersection(data.keys()), f"PII leak: {pii_fields.intersection(data.keys())}"


@pytest.mark.asyncio
async def test_get_unknown_device_returns_404(async_client: AsyncClient):
    resp = await async_client.get("/api/v1/devices/no-such-device-xyz", headers={"X-API-Key": "test-key"})
