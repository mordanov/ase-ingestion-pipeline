...
def _mock_provider_result(provider_id: str = "service1") -> ProviderResult:
    return ProviderResult(
        ...
        recommendations=[
            RawRecommendation(
                short_text="Walk more", detail=None, normalised_score=500.0, provider_id=provider_id
            )
        ],
        error=None,
        duration_ms=50,
...
@pytest.mark.asyncio
async def test_tier_upgrades_to_silver_at_1000_cumulative(
    async_client: AsyncClient, db_session, seeded_device
):
    await db_session.execute(
        update(Device)
        .where(Device.device_id == seeded_device.device_id)
