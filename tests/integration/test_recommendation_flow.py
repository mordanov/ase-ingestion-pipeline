...
async def test_recommendations_returns_200_with_results(async_client: AsyncClient, seeded_device):
    with (
        patch(
            "src.recommendation.adapters.service1_adapter.Service1Adapter.get_recommendations",
            new_callable=AsyncMock,
            return_value=_mock_provider_result("service1"),
        ) as _,
        patch(
            "src.recommendation.adapters.service2_adapter.Service2Adapter.get_recommendations",
            new_callable=AsyncMock,
            return_value=_mock_provider_result("service2"),
        ) as _,
    ):
        ...
        patch(
            ...
            return_value=ProviderResult(
                ...
                recommendations=[
                    ...
                    RawRecommendation(
                        short_text="Sleep better",
                        detail=None,
                        normalised_score=700.0,
                        provider_id="service1",
                    ),
                ],
                error=None,
...
@pytest.mark.asyncio
async def test_recommendations_returns_402_on_zero_credits(
    async_client: AsyncClient, db_session, seeded_device
):
...
@pytest.mark.asyncio
async def test_recommendations_providers_succeeded_in_response(
    ...
    with (
        patch(
            "src.recommendation.adapters.service1_adapter.Service1Adapter.get_recommendations",
            new_callable=AsyncMock,
            return_value=_mock_provider_result("service1", 600.0),
        ),
        patch(
            "src.recommendation.adapters.service2_adapter.Service2Adapter.get_recommendations",
            new_callable=AsyncMock,
            return_value=_mock_provider_result("service2", 400.0),
        ),
    ):
        resp = await async_client.post(
            f"/api/v1/devices/{seeded_device.device_id}/recommendations",
