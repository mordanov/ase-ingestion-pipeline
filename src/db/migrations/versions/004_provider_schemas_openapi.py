...
def upgrade() -> None:
    ...
    if not _column_exists("provider_schemas", "openapi_schema"):
        op.add_column("provider_schemas", sa.Column(
            "openapi_schema", postgresql.JSONB(), nullable=True
        ))
...
def downgrade() -> None:
    op.add_column("provider_schemas", sa.Column("schema_type", sa.String(32), nullable=False, server_default="service1_schema"))
