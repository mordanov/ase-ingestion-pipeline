...
from src.db.models.ml_trained_model import TrainedModel, ModelDeploymentStatus
...
async def get_summary_report(db: DbSession, settings: AppSettings) -> Any:
    ...
    active_rows = await db.execute(
        select(func.count(func.distinct(TelemetryEvent.device_id)))
    ...
    quarantined_24h = (await db.execute(
        select(func.count(QuarantineRecord.id))
    ...
    last_job = (await db.execute(
        select(TrainingJob).order_by(TrainingJob.started_at.desc()).limit(1)
    )).scalar_one_or_none()

    active_models_rows = (await db.execute(
        select(TrainedModel)
        .where(TrainedModel.deployment_status == ModelDeploymentStatus.active)
    )).scalars().all()
