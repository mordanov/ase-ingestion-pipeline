...
async def get_provider_adapters(http_client: HttpClient, settings: AppSettings, db: DbSession):
    ...
    rows = (await db.execute(_select(ProviderSchema).where(ProviderSchema.is_active == True))).scalars().all()
    for row in rows:
        adapters.append(DynamicAdapter(
            http_client, row.name, row.endpoint_url,
            row.request_mapping or {}, row.response_mapping or {},
        ))
...
def get_twin_adapter(settings: AppSettings):
    ...
    return IotCoreAdapter(settings.aws_region, settings.aws_iot_endpoint, settings.aws_iot_policy_name, settings.aws_iot_thing_type)
