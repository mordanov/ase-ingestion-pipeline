    ...
    <section className="bg-white border border-slate-200 rounded-xl p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Runs the full extract → feature-engineer → train → evaluate → register pipeline for both
          the re-ranker and anomaly-detector models.
        </p>
      </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm text-green-700 font-medium">
            ...
            {job.started_at && job.ended_at && (
              <span className="text-slate-400 font-normal">in {elapsed(job.started_at)}</span>
            )}
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <MetricCard
              label="Re-ranker NDCG@10"
              value={fmt(job.reranker_ndcg_at_10)}
              good={job.reranker_ndcg_at_10 != null && job.reranker_ndcg_at_10 >= 0.7}
            />
            <MetricCard
              label="Anomaly Detector F1"
              value={fmt(job.anomaly_detector_f1)}
              good={job.anomaly_detector_f1 != null && job.anomaly_detector_f1 >= 0.7}
            />
    ...
    <section className="bg-white border border-slate-200 rounded-xl p-6 space-y-4">
      <div>
        <h2 className="text-base font-semibold text-slate-800">
          Step 2 — Download TFLite Model Package
        </h2>
        <p className="text-sm text-slate-500 mt-0.5">
          The on-device ZIP contains both the re-ranker and anomaly-detector TFLite models along
          with a <code className="bg-slate-100 px-1 rounded text-xs">manifest.json</code> describing
      ...
      {notFound && (
        <div className="rounded-lg bg-slate-50 border border-slate-200 px-4 py-3 text-sm text-slate-500 space-y-2">
          ...
          <p className="text-xs">
            Complete Step 1 to build the first model and auto-generate a distribution package.
          </p>
          <button
            onClick={() => {
              qc.invalidateQueries({ queryKey: ['ml-package-latest'] })
              refetch()
            }}
      ...
      {mutation.isSuccess && mutation.data && (
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500">
            <span>
              Device: <span className="font-mono text-slate-700">{mutation.data.device_id}</span>
            </span>
            <span>
              {mutation.data.providers_succeeded.length}/{mutation.data.providers_called.length}{' '}
              providers
            </span>
  ...
  if (status === 404)
    return (
      <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg px-4 py-3">
        Device <code className="font-mono">{deviceId}</code> not found. Register it first via the
        Credits page.
      </div>
    )
  if (status === 402)
...
function RecommendationCard({ item, rank }: { item: RecommendationItem; rank: number }) {
  ...
  return (
    <li
      className={`border rounded-lg px-4 py-3 space-y-1.5 ${item.anomaly_suppressed ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white'}`}
    >
          ...
          {isScored ? (
            <span
              ...
              style={{
                ...
                borderColor: scoreColor(score!).border,
            ...
            <span
              className="text-xs text-slate-400 px-2 py-0.5 rounded-full border border-slate-200 bg-slate-50"
              title="Cold-start: no embedding yet"
            >
              cold-start
            </span>
          )}
      ...
      {item.detail && <p className="text-xs text-slate-500 ml-6">{item.detail}</p>}
...
function TierPill({ tier }: { tier: string }) {
  ...
  return (
    <span
      className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${colors[tier] ?? 'bg-slate-100 text-slate-600'}`}
    >
      {tier}
    </span>
  )
